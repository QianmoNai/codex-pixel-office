...
$ListenHost = if ($env:CODEX_PIXEL_HOST) { $env:CODEX_PIXEL_HOST } else { "127.0.0.1" }
$ListenPort = if ($env:CODEX_PIXEL_PORT) { $env:CODEX_PIXEL_PORT } else { "8765" }
$ServerArgs = @(
    (Join-Path $RootDir "server.py"),
    "--host", $ListenHost,
    "--port", $ListenPort
)
if ($env:CODEX_PIXEL_NO_BROWSER -ne "1") {
    $ServerArgs += "--open-browser"
}
$ServerArgs += @($args)

& $PythonPath @PythonPrefix @ServerArgs
exit $LASTEXITCODE

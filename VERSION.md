...
`v0.1.0` 是 Codex Pixel Office 的首个正式文档化版本。它已经具备可运行的桌面端、浏览器端、会话可视化、角色动作、会话续聊、模型选择和自动化测试，但对外接口与 Codex 内部状态格式仍可能变化，因此暂定为技术预览版本。

> [!NOTE]
> `server.py` 中 HTTP `Server` 响应头使用的产品标识，以及 Desktop Entry 中的 `Version=1.0` 字段，不代表应用的语义化版本；本文件中的 `v0.1.0` 才是当前产品版本声明。
...
- GTK4 / WebKitGTK 6 桌面窗口、Linux 应用菜单安装和浏览器运行模式。
- 针对会话解析、HTTP 安全边界、聊天生命周期和桌面启动的自动化测试。
...
| 桌面模式 | Linux 图形环境；PyGObject、GTK4、WebKitGTK 6 |
| 浏览器模式 | 现代浏览器；状态服务仅需 Python 标准库 |
| 默认网络边界 | `127.0.0.1` 回环地址 |

本项目没有声明对所有 Codex CLI 历史版本、所有 Linux 发行版或非 Linux 桌面环境提供兼容保证。
...
3. **桌面封装目前面向 Linux。** 浏览器模式的后端逻辑更通用，但安装脚本和桌面入口使用 Linux/XDG 约定。
...
2. 运行 `python3 -m unittest discover -s tests -v` 并确认全部通过。
3. 检查桌面模式和浏览器模式的最小启动流程。
